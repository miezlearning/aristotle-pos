...
- Lisensi = ranah Owner. Kasir tidak boleh membuka modal aktivasi, melihat, menyalin, atau mengganti key (`openActivateLicenseModal`, render/toggle/copy, dan submit aktivasi wajib menolak peran kasir; render box key punya penjagaan lapis kedua).
- Lisensi terikat storeId di cloud, BUKAN per device — HP staf dalam satu toko berbagi entitlement yang sama secara sah. Yang dilarang = key yang sama dipakai tokoId berbeda (ditolak `claimedByStoreId`, baik saat aktivasi maupun restore).
- Bedakan vonis definitif vs gagal teknis: `revoked` / dipakai-toko-lain / tak-terdaftar (`definitive: true`) DITEGAKKAN saat online; gagal jaringan/izin = fail-open (percaya cache). Jangan pernah menukar keduanya.
- Kuota demo dihitung dari `(state.transactions || []).length` dan ditegakkan DUA kali: saat modal bayar dibuka DAN saat commit (`completeTransaction`). Jangan pakai field yang tidak ada di `state`.
- QRIS tanpa bukti bayar tidak boleh jadi omzet: tanpa payload terpasang = tolak + arahkan setup; dengan payload = wajib dialog konfirmasi dana masuk.
- Semua mutasi destruktif (hapus transaksi/riwayat, simpan/hapus/import menu) + diskon = Owner-only. Proteksi di level fungsi, bukan cuma sembunyikan tombol (konsol = pintu belakang).
